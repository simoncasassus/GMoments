...
def dgauss_wbase(x, a1, mu1, sigma1, a2, mu2, sigma2, base_a, base_b):
    ...
    model = baseline + gaussian(x, a1, mu1, sigma1) + gaussian(x, a2, mu2, sigma2)
    return model
...
def fitter(alos):
    ...
    if isinstance(MaskCube, np.ndarray):
        ...
        lineofsight_spectrum[masklos] = 0.0
    ...
    if LocalNoise:
        ...
        localnoise = selected_losspectrum[
            (selected_velocities < v0_init - 1.0)
            | (selected_velocities > v0_init + 1.0)
        ]
        localrmsnoise = np.std(localnoise)

    if DoClip:
        lineofsight_spectrum[np.where(lineofsight_spectrum < 0)] = 0.0
        selected_losspectrum[np.where(selected_losspectrum < 0)] = 0.0
    ...
    limit_a1 = [0.0, 5.0 * Amp_init]  # Bounds for pars
    limit_mu1 = [velocities.min() - 1.0, velocities.max() + 1.0]
    limit_sigma1 = [dv / 3.0, sigma_max]
    limit_base_a = [-maxslope, maxslope]
    limit_base_b = [0.0, selected_losspectrum.max()]
    ...
    if DoBaseline:
        ...
        fallback_errors = np.array(
            (
                fallback_error_a1,
                fallback_error_mu1,
                fallback_error_sigma1,
                fallback_error_base_a,
    ...
    try:
        if LocalNoise:
            popt, pcov = op.curve_fit(
                ...
                xdata,
                ydata,
                sigma=np.ones(len(ydata)) * localrmsnoise,
                p0=p0,
        ...
        if np.any(np.isnan(popt)) or np.any(np.diag(pcov) < 0.0):
    ...
    if DGauss:
        ...
        if Randomize:
            ...
            v0_init2 = g_v0 + 3.0 * dv * (np.random.random() - 0.5)  #
        ...
        limit_a2 = [0.0, Amp_init]  # Bounds for pars
        limit_mu2 = [velocities.min() - 1.0, velocities.max() + 1.0]
        limit_sigma2 = [dv / 3.0, sigma_max]
        ...
        if CommonSigma:
            if DoBaseline:
                ...
                func = lambda x, a1, mu1, sigma1, a2, mu2: dgauss_wbase(
                    x, a1, mu1, sigma1, a2, mu2, sigma1, base_a_init, base_b_init
                )
            ...
            if DoBaseline:
                ...
                func = lambda x, a1, mu1, sigma1, a2, mu2, sigma2: dgauss_wbase(
                    x, a1, mu1, sigma1, a2, mu2, sigma2, base_a_init, base_b_init
                )
                p0 = np.array(
                    [a1_init, mu1_init, sigma1_init, a2_init, mu2_init, sigma2_init]
                )
                bounds = (
                    limit_a1,
                    limit_mu1,
    ...
    if DGauss:
        ...
        if ComputeG8:
            ...
            f_vpeak = lambda vmax: neggaussfit(
                vmax, g_amp, g_v0, g_sigma, g2_amp, g2_v0, g2_sigma
            )
    ...
    gmom_0 = np.sqrt(2.0 * np.pi) * g_sigma * g_amp  # abs(simps(fit1,velocities))
    ...
    gmom_0_simps = abs(simpson(fit2, velocities))
    ...
    if DGauss:
        ...
        if DoQuad:
            ...
            gmom_0_quad = simpssign * simpson(evalfunc, velocities)
        ...
        if DoQuad:
            ...
            Num_mom1 = simpssign * simpson(evalfunc, velocities)
        ...
        if DoQuad:
            ...
            Num_mom2 = simpssign * simpson(evalfunc, velocities)
    ...
    if ViewSingleSpectrum:
        ...
        fileout_spectrum = (
            "LOSspectrum_" + str(LOS["dalpha"]) + "_" + str(LOS["ddelta"]) + ".dat"
        )
        np.savetxt(fileout_spectrum, save_prof)

        theplot = ax1.plot(velocities, fit1, label="model", lw=4.0, alpha=0.5)
    ...
    if velocities[1] < velocities[0]:
        sign = -1
    Smom_0 = sign * simpson(lineofsight_spectrum[i0:i1], velocities[i0:i1])
    Smom_1 = sign * simpson(
        lineofsight_spectrum[i0:i1] * velocities[i0:i1], velocities[i0:i1]
    )
...
def exec_Gfit(
    ...
    if cubemask:
        ...
        if InvertMaskCube:
            MaskCube[(MaskCube >= 1.0)] = 2.0
            MaskCube[(MaskCube < 1.0)] = 1.0
            MaskCube[(MaskCube == 2)] = 0.0
    ...
    nu = (np.arange(datahdr["NAXIS3"]) - (datahdr["CRPIX3"] - 1)) * datahdr[
        "CDELT3"
    ] + datahdr["CRVAL3"]
    ...
    icenter = int(datahdr["CRPIX1"] - 1.0)
    if zoom_area > 0.0:
        halfside_pix = int(zoom_area / (3600.0 * datahdr["CDELT2"]))
    ...
    if singleLOS:
        ...
        i = int(headcube["CRPIX1"] + dalpha / (3600.0 * headcube["CDELT1"]))
        j = int(headcube["CRPIX1"] + ddelta / (3600.0 * headcube["CDELT2"]))
    ...
    if not "BMAJ" in headim.keys():
        ...
        if len(hdulist) > 1:
            ...
            bmin = beamdata[0][1]
            bpa = beamdata[0][2]
            headim["BMAJ"] = bmaj / 3600.0
            headim["BMIN"] = bmin / 3600.0
            headim["BPA"] = bmaj
    ...
    if ShrinkCanvas:
        ...
        if isinstance(MaskCube, np.ndarray):
            pf.writeto(
                workdir + "/" + "maskcube.fits", MaskCube, headcube, overwrite=True
            )
    ...
    if DGauss:
        ...
        pf.writeto(
            workdir + "/" + "im_g2_sigma.fits", im_g2_sigma, head2, overwrite=True
        )
        pf.writeto(
            workdir + "/" + "im_g2_sigma_e.fits", im_g2_sigma_e, head2, overwrite=True
        )
        pf.writeto(workdir + "/" + "im_gmom_8.fits", im_gmom_8, headim, overwrite=True)

        pf.writeto(workdir + "/" + "im_gmom_0.fits", im_gmom_0, head1, overwrite=True)
        pf.writeto(workdir + "/" + "im_gmom_1.fits", im_gmom_1, head2, overwrite=True)
        pf.writeto(workdir + "/" + "im_gmom_2.fits", im_gmom_2, head2, overwrite=True)

        if StoreModel:
            pf.writeto(
                workdir + "/" + "modelcube_g1.fits",
                modelcube_g1,
                headcube,
                overwrite=True,
            )
            pf.writeto(
                workdir + "/" + "modelcube_g2.fits",
                modelcube_g2,
                headcube,
                overwrite=True,
            )
    ...
    if StoreModel:
        pf.writeto(
            workdir + "/" + "modelcube.fits", modelcube, headcube, overwrite=True
        )
