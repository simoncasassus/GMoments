...
def fitter(alos):
    ...
    ## take the error as the rmsnoise far from the line
        localnoise = selected_losspectrum[(selected_velocities<v0_init-1.) | (selected_velocities>v0_init+1.)]
    ...
    try:
        if LocalNoise:
            popt, pcov = op.curve_fit(func, xdata, ydata,sigma=np.ones(len(ydata))*localrmsnoise,p0=p0,bounds=[lowlimits,uplimits],absolute_sigma=True)
        else:
            ...
            if (rmsnoise < 0.):
                popt, pcov = op.curve_fit(func, xdata, ydata,p0=p0,bounds=[lowlimits,uplimits],absolute_sigma=False)
            else:
                popt, pcov = op.curve_fit(func, xdata, ydata,sigma=np.ones(len(ydata))*rmsnoise,p0=p0,bounds=[lowlimits,uplimits],absolute_sigma=True)
    ...
    setofparamnames1=['a1','mu1','sigma1'] #,'a2','mu2','sigma2','base_a','base_b']
    ...
    Smom_1 = sign*simps(lineofsight_spectrum[i0:i1]*velocities[i0:i1],velocities[i0:i1])
    ...
    if (abs(Smom_0) > 0):
        Smom_1 /= Smom_0
        if (Smom_0 > 0):
            var = sign*simps(lineofsight_spectrum[i0:i1]*(velocities[i0:i1] - Smom_1)**2,velocities[i0:i1])
            if (var > 0):
    ...
    passresults = [i,j,gmom_0,g_amp,g_amp_e,g_v0,g_v0_e,g_sigma,g_sigma_e,Smom_0,Smom_1,Smom_2,Smom_8,fiterror,gaussfit1]
    if DGauss:
        passresults.extend([gaussfit2,g2_amp,g2_amp_e,g2_v0,g2_v0_e,g2_sigma,g2_sigma_e,vpeak,gmom_0_quad,gmom_1_quad,gmom_2_quad])
    if (DoBaseline): 
        passresults.extend([base_a,base_b,baseline])
    passresults.extend([Smax,])
